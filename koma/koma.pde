...
void draw() 
{
	...
	case 1:
		...
		if(!koma.is_falled()) {
			if(is_on_plate) {
				score++;
				if(koma.is_just_landing()) {
					javascript.sePlayLanding();
				}
			}
			else {
				koma.fall();
...
class CKoma {
	...
	void jump(float vz)
	{
		if(m_jumpCnt < 3) {
			m_vz = vz;
			m_jumpCnt++;
			javascript.sePlayJump();
		}
	}
	boolean is_falled() { return m_is_falled; }
	boolean is_just_landing() { return m_is_just_landing; }
	void fall()
	{
		m_is_falled = true;
		javascript.sePlayFalling();
	}
...
class CPlate {
	...
	void setup_random(int r)
	{
		...
		float x = sin(rot) * r + 480 / 2;
		float y = cos(rot) * r + 480 / 2;
		m_cx = x;
		m_cy = y;
		m_w = 128 + random(288);
		m_h = 128 + random(288);
